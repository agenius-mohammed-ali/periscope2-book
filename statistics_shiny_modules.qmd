...
and it can be calculated using `mean` function in `R`: Ex1: Mean height of trees is:
...
Median value is the value that divides feature data into two equal sets. The median calculated differently based on observations number as follow:

$$
Y\_{ij} = \\begin{dcases\*} \\sum\_{k \\sim i}y\_{ij}, & if \$ i = j \$,\\\\ {-y\_{ij}}, & if \$ i \\ne j \$ and \$ i \\sim j \$,\\\\ 0, & otherwise. \\end{dcases\*}\
